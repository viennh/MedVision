#!/bin/bash
#SBATCH --job-name=llavamed-detect
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=logs/llavamed-detect_%j.out
#SBATCH --error=logs/llavamed-detect_%j.err

# ============================================================
# LLaVA-Med Detection Evaluation — UCF Newton
# ============================================================
...
if [ ! -d "${CONDA_HOME}/envs/${ENV_NAME}" ]; then
    conda create --prefix "${CONDA_HOME}/envs/${ENV_NAME}" python==3.10 -y
fi
