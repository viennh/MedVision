#!/bin/bash
#SBATCH --job-name=medgemma-detect
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=logs/medgemma-detect_%j.out
#SBATCH --error=logs/medgemma-detect_%j.err

# ============================================================
# MedGemma Detection Evaluation — UCF Newton
# ============================================================
...
batch_size_per_gpu=1  # ~16GB-safe for MedGemma+SigLIP; raise on large GPUs if VRAM allows
...
$PYTHON -m pip install -r "${benchmark_dir}/requirements/requirements_eval_medgemma.txt" --no-deps
